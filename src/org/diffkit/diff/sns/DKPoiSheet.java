...
public class DKPoiSheet extends DKAbstractSheet {
	...
	public DKPoiSheet(File file_, String name_, boolean isSorted_,
	...
	protected DKTableModel createModelFromSheet() throws IOException {
		...
		for (int i = 1; i < columnModels.length; i++)
			columnModels[i] = new DKColumnModel(i, columNames[i - 1],
					columnTypes.get(i - 1));
		int[] key = { 0 };
	...
	private String[] getColumnNames(int columnCount_) throws IOException {
		...
		if (columnCount_ != headerWidth)
			throw new RuntimeException(String.format(
					"headerWidth->%s does not match columnCount_->%s",
					headerWidth, columnCount_));
		...
		if (requestedName != null) {
			...
			if (_sheet == null)
				throw new IOException(String.format(
						"couldn't find sheet->'%s' in workbook->'%s'",
						requestedName, this.getFile()));
		} else {
	...
	private Workbook getWorkbook() throws IOException {
		if (_workbook != null)
			return _workbook;
		try {
			_workbookInputStream = DKStreamUtil.ensureBuffered(FileUtils
					.openInputStream(this.getFile()));
			...
			for (int j = 0; j < width; j++) {
				...
				if (_isDebugEnabled) {
					_log.debug(String.format(
							"cell->%s formatString->%s format->%s", cell
									.getColumnIndex(), cell.getCellStyle()
				...
				if (_isDebugEnabled)
					_log.debug("cellType->{} columnType->{}", cellType,
							columnType);
				if (columnType == null)
	...
	private static Type getTypeForNumericCell(Cell cell_) {
		...
		if (type != null)
			return type;
		type = mapTypeForFormatString(cell_.getCellStyle()
				.getDataFormatString());
		if (type == null)
			type = Type.DECIMAL;
